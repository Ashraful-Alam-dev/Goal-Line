...
function Stat({ label, value, highlight }: { label: string; value: React.ReactNode; highlight?: boolean }) {
  return (
    <div className="rounded-lg bg-secondary/40 border border-border/60 p-2.5">
      ...
      <p className={`font-semibold tabular ${highlight ? "text-amber-500" : "text-foreground"}`}>
        {value}
      </p>
    </div>
  );
...
export function BetCard({ bet, admin = false }: Props) {
  ...
  const statusMeta =
    bet.status === "WON"
      ...
      : bet.status === "LOST"
      ? { icon: TrendingDown, className: "gap-1 bg-rose-600/15 text-rose-500 border border-rose-600/30" }
      : { icon: Clock, className: "gap-1 bg-secondary text-muted-foreground" };

  const StatusIcon = statusMeta.icon;

  return (
    <Card className="p-5 space-y-4 transition-all duration-300 hover:-translate-y-0.5 hover:shadow-[0_0_24px_-10px_rgba(194,112,10,0.25)]">
